...
def train(train_loader, optimizer, model, criterion, epoch, FDL_loss):
    ...
    for iteration, batch in enumerate(train_loader, 1):
        ...
        for i in range(3):
            ...
            fdl_loss = FDL_loss(SR, label)
            loss = criterion(SR, label)+ criterion(label_LR, frist_result)+0.1*fdl_loss
...
def val(val_loader, model, epoch):
    ...
    with torch.no_grad():
        for iteration, batch in enumerate(val_loader, 1):
            ...
            SR,frest = model( input,input1,splitted_images)
